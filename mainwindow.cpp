...
void MainWindow::Odswiez()
{
    ...
    scena->addLine(punkt1.x(),punkt1.y(),punkt2.x(),punkt2.y());
    scena->addLine(punkt3.x(),punkt3.y(),punkt2.x(),punkt2.y(), QPen(Qt::red));

    scena->addRect( zakazane_pole, QPen(Qt::red) );
...
void MainWindow::spadanie()
{
    ...
    for (int i = 0; i < ILOSC_ELEMENTOW; i++)
    {
        ...
        if( scena->height() > klocki[i].wsk->boundingRect().height() + klocki[i].wsk->pos().y()) //sprawdza czy elemnt lezy na podlodze
        {
